...
struct Globals{
  ...
  static constexpr unsigned int primes[8] = {2,3,5,7,11,13,17,19};
  static const unsigned int numPrimes = sizeof(primes) / sizeof(primes[0]);
  ...
  static constexpr unsigned int primeRepresentationLength = (exponentMax-exponentMin)*numPrimes;
} globals;
...
Vvi build_input() {
  ...
  Vvi vvi;
  for(unsigned int i = 0; i < globals.numPrimes; i++) {//this index determines which prime
